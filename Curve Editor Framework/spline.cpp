	...
	switch(style)
	{
        ...
        case HermiteClamped:
            ...
            // ADD YOUR CODE HERE
			{
				...
				//Get d
				vec3 startPos = interpPoints[0]->getPosition() - startPoint->getPosition();
				d(0,0) = startPos[0]; d(0,1) = startPos[1]; d(0,2) = startPos[2];
				for(int i = 2; i < totalPoints; i++)
				{
					vec3 v = (interpPoints[i] - interpPoints[i-2]) * 3;
				...
				{
					Node * newControlPoint = new Node(graph, ControlPoints);
					newControlPoint->setPosition(interpPoints[i]->getPosition() + vec3(C(i,0), C(i,1), C(i,2)));
					controlPoints.append(newControlPoint);
				}
			}
            break;
        case HermiteNatural:
            ...
            // ADD YOUR CODE HERE
				...
				//Get d
				vec3 startPos = interpPoints[1]->getPosition() - interpPoints[0]->getPosition();
				d(0,0) = startPos[0]; d(0,1) = startPos[1]; d(0,2) = startPos[2];
				for(int i = 2; i < totalPoints; i++)
				{
					vec3 v = (interpPoints[i] - interpPoints[i-2]) * 3;
				...
				vec3 endPos = interpPoints[totalPoints-1]->getPosition() - interpPoints[totalPoints-2]->getPosition();
				d(totalPoints-1,0) = endPos[0]; d(totalPoints-1,1) = endPos[1]; d(totalPoints-1,2) = endPos[2];
